 ...
 * (SSE docs/dev/vtt-integration-design.md section 11 (shared with Star System Explorer, kept identical))
 *
...
const STORAGE_KEY = 'mappadux:ice_servers';
...
const MANAGED_CACHE_KEY = 'mappadux:managed_ice';
...
/**
 * v2.19.14 — does this ICE configuration actually work? Gathers candidates from
 * the browser and reports what came back, so a GM can find out at setup time
 * rather than from a player who cannot join mid-session.
 *
...
/**
 * v2.19.14 — what to tell the GM when a joiner's connection was blocked.
 *
 ...
 *                  one UDP-only community TURN, which a locked-down or
 *                  IPv6-only network defeats. A turns:443 relay is the fix.
...
function sanitise(arr: unknown): IceServerEntry[] | null {
  ...
  for (const e of arr) {
    ...
    const u = (e as IceServerEntry).username, c = (e as IceServerEntry).credential;
    if (typeof u === 'string') entry.username = u;
    if (typeof c === 'string') entry.credential = c;
    out.push(entry);
  }
  return out;
}
